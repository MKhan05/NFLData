...
.remove
model2 = glm(rating~completions+attempts+percent.complete+attempts.game+yards+average.yards+yards.per.game+td+int+first.down+first.down.percent+long+twenty.plus+forty.plus+sacks, data=.remove)
plot(model2)
...
```{r}
NFL_reg = lm(rating~completions+attempts+percent.complete+yards+td+int+first.down+first.down.percent+long+twenty.plus+forty.plus+sacks, data=qbrating2)
summary(NFL_reg)

tidy(NFL_reg) %>% arrange(p.value)
```

Only td, int, first.down and first.down.percent have p-values below 0.05. What if we remove sacks (highest p-value)?

```{r}
NFL_reg2 = lm(rating~completions+attempts+percent.complete+yards+td+int+first.down+first.down.percent+long+forty.plus, data=qbrating2)
summary(NFL_reg2)

tidy(NFL_reg2) %>% arrange(p.value)
```

The value for R-squared (0.855) does not change by much, although we do see a shift in p-values. What if we remove forty.plus now?

```{r}
NFL_reg3 = lm(rating~completions+attempts+percent.complete+yards+td+int+first.down+first.down.percent+long, data=qbrating2)
summary(NFL_reg3)
...
```{r}
NFL_reg4 = lm(rating~attempts+percent.complete+yards+td+int+first.down+first.down.percent+long, data=qbrating2)
summary(NFL_reg4)
...
```{r}
NFL_reg5 = lm(rating~attempts+percent.complete+yards+td+int+first.down+first.down.percent, data=qbrating2)
summary(NFL_reg5)
...
```{r}
NFL_reg6 = lm(rating~attempts+yards+td+int+first.down+first.down.percent, data=qbrating2)
summary(NFL_reg6)
...
```{r}
NFL_reg7 = lm(rating~yards+td+int+first.down+first.down.percent, data=qbrating2)
summary(NFL_reg7)
