...
.remove = qbrating[,-(1:4)]
.remove
.remove.stack = stack(.remove)
.remove.stack
model = lm(values~ind, data=.remove.stack)
plot(model)
residuals = resid(model)
plot(residuals)
#Might need to change scaling?
```
...
## Using built in R function to double check backwards elimination:
```{r}
step(NFL_reg, direction="backward")
```
